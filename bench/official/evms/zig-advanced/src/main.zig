...
pub fn main() !void {
    ...
    while (run < num_runs) : (run += 1) {
        ...
        // Analyze code with jump analysis for advanced interpreter
        const analysis = try evm.frame.CodeAnalysis.analyze_bytecode_blocks(allocator, code);
        const analysis_ptr = try allocator.create(evm.frame.CodeAnalysis);
        analysis_ptr.* = analysis;
        contract.analysis = analysis_ptr;
        
        // Check if we should use advanced interpreter
        if (contract.analysis != null and 
            contract.analysis.?.block_count > 0 and
            contract.analysis.?.jump_analysis != null) {
            
            // Generate instruction stream
            var stream = try evm.advanced_interpreter.instruction_stream.generate_instruction_stream(
                allocator,
                code,
                contract.analysis.?,
            );
            ...
            std.debug.print("About to execute contract using ADVANCED interpreter at address: {any}\n", .{contract_address});
            std.debug.print("Contract code length: {}\n", .{code.len});
            std.debug.print("Calldata: 0x{x}\n", .{std.fmt.fmtSliceHexLower(calldata)});
            std.debug.print("Block count: {}, Jump analysis: {} static jumps\n", .{
                contract.analysis.?.block_count,
                contract.analysis.?.jump_analysis.?.static_jump_count,
            });
            
            const result = try evm.advanced_interpreter.execute_advanced.execute_advanced(&vm, &frame, &stream);
            ...
            // Fall back to regular interpreter if advanced features not available
            std.debug.print("Advanced interpreter not available, falling back to regular interpreter\n", .{});
