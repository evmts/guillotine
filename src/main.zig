...
fn printHelp() !void {
    ...
    try stdout.print(
        \\Guillotine - High-Performance Ethereum Virtual Machine
        \\
        \\This is the main CLI for the Guillotine EVM implementation and benchmarking suite.
        \\
        \\Usage: Guillotine [OPTIONS]
        \\
        \\Options:
        \\  -h, --help                 Display this help and exit
        \\  -e, --evm <NAME>           EVM implementation to benchmark (default: zig)
        ...
        \\
        \\Examples:
        \\  Guillotine                         Run benchmarks with Zig EVM
        \\  Guillotine -e zig -n 50            Run 50 iterations per test case
        \\  Guillotine --export json           Export results to JSON
        \\  Guillotine --export markdown       Export results to Markdown
        \\  Guillotine --compare --export markdown     Compare all EVMs and export
        \\  Guillotine --compare --js-runs 1          Compare EVMs with only 1 run for JavaScript
        \\  Guillotine --diff erc20-transfer          Run differential trace comparison
        \\  Guillotine --diff snailtracer --diff-output traces    Custom output directory
        \\  Guillotine --detailed                     Run detailed performance analysis with metrics
        \\  Guillotine --detailed --perf-output results    Custom output directory for perf reports
        \\  Guillotine --crypto                       Run crypto benchmarks between Zig and Rust implementations
        \\
        \\Available EVM Implementations:
        \\  zig        - Guillotine Zig EVM (default)
        \\  zig-call2  - Guillotine with tailcall dispatch
        \\  revm       - REVM (Rust)
        \\  ethereumjs - EthereumJS (JavaScript via Bun)
        \\  geth       - Geth (Go)
        \\  evmone     - evmone (C++)
        \\
    , .{});
