...
pub fn Uint(comptime bits: usize, comptime limbs: usize) type {
    return struct {
        ...
        pub const ONE: Self = if (bits == 0) ZERO else Self.from_u64(1);
        ...
        pub fn overflowing_add(self: Self, rhs: Self) struct { result: Self, overflow: bool } {
            ...
            for (0..limbs) |i| {
                const add_result = @addWithOverflow(result.limbs[i], rhs.limbs[i]);
                const carry_result = @addWithOverflow(add_result[0], @intFromBool(carry));
                result.limbs[i] = carry_result[0];
                carry = add_result[1] or carry_result[1];
            }
            
            const overflow = carry or (limbs > 0 and result.limbs[limbs - 1] > MASK);
        ...
        pub fn overflowing_sub(self: Self, rhs: Self) struct { result: Self, overflow: bool } {
            ...
            for (0..limbs) |i| {
                const sub_result = @subWithOverflow(result.limbs[i], rhs.limbs[i]);
                const borrow_result = @subWithOverflow(sub_result[0], @intFromBool(borrow));
                result.limbs[i] = borrow_result[0];
                borrow = sub_result[1] or borrow_result[1];
            }
            
            const overflow = borrow;
            return .{ .result = result.masked(), .overflow = overflow };
        }
        ...
        pub fn overflowing_mul(self: Self, rhs: Self) struct { result: Self, overflow: bool } {
            ...
            for (0..limbs) |i| {
                ...
                for (0..limbs) |j| {
                    ...
                    const prod = @as(u128, self.limbs[i]) * @as(u128, rhs.limbs[j]) + @as(u128, result.limbs[i + j]) + carry;
        ...
        pub fn div_rem(self: Self, divisor: Self) struct { quotient: Self, remainder: Self } {
        ...
        pub fn get_bit(self: Self, index: usize) bool {
            ...
            return (self.limbs[word_index] & mask) != 0;
        }
        ...
        pub fn count_zeros(self: Self) u32 {
            if (bits == 0) return 0;
            return @intCast(bits) - self.count_ones();
        }
        ...
        pub fn set_byte(self: Self, index: usize, value: u8) Self {
            ...
            var result = self;
            const shift = @intCast(u6, byte_index * 8);
            const mask = ~(@as(u64, 0xFF) << shift);
            result.limbs[limb_index] = (result.limbs[limb_index] & mask) | (@as(u64, value) << shift);
            return result;
        }
        ...
        pub fn next_power_of_two(self: Self) ?Self {
            ...
            const ones = @intCast(u32, bits) - self.leading_zeros();
        ...
        fn format_binary(self: Self, writer: anytype) !void {
            ...
            while (i > 0) : (i -= 1) {
                ...
                if (!started) {
                    const leading_zeros = @clz(limb);
                    var j: u6 = 63 - @as(u6, @intCast(leading_zeros));
                    started = true;
                    while (j < 64) : (j -%= 1) {
                        const bit = (limb >> j) & 1;
        ...
        fn format_hex(self: Self, writer: anytype, uppercase: bool) !void {
            ...
            while (i > 0) : (i -= 1) {
                ...
                if (!started) {
                    const leading_zeros = @clz(limb);
                    const nibbles_to_skip = @divFloor(leading_zeros, 4);
                    started = true;
        ...
        pub fn to_le_bytes(self: Self, bytes: []u8) void {
            const byte_len = @min(bytes.len, (bits + 7) / 8);
            const full_limbs = byte_len / 8;
            const extra_bytes = byte_len % 8;
        ...
        pub fn from_le_bytes(bytes: []const u8) Self {
            ...
            const byte_len = @min(bytes.len, max_bytes);
            
            var byte_idx: usize = 0;
            var limb_idx: usize = 0;
            
            while (byte_idx < byte_len and limb_idx < limbs) {
                var limb: u64 = 0;
                const bytes_in_limb = @min(8, byte_len - byte_idx);
        ...
        pub fn div_ceil(self: Self, rhs: Self) Self {
            ...
            const q_r = self.div_rem(rhs);
            if (q_r.remainder.is_zero()) {
                return q_r.quotient;
            } else {
                return q_r.quotient.add(Self.ONE);
            }
        }
        ...
        pub fn next_power_of_two(self: Self) Self {
            return self.checked_next_power_of_two() orelse @panic("overflow in next_power_of_two");
        }
        ...
        pub fn checked_next_power_of_two(self: Self) ?Self {
            ...
            const exp = self.bit_len();
            if (exp >= bits) {
                return null;
            }
            return Self.ONE.shl(exp);
        }

        pub fn next_multiple_of(self: Self, rhs: Self) Self {
            return self.checked_next_multiple_of(rhs) orelse @panic("overflow or division by zero");
        }

        pub fn checked_next_multiple_of(self: Self, rhs: Self) ?Self {
            if (rhs.is_zero()) {
                return null;
            }
            const q_r = self.div_rem(rhs);
            ...
            const q = q_r.quotient.checked_add(Self.ONE) orelse return null;
            return q.checked_mul(rhs);
        }

        pub fn saturating_add(self: Self, rhs: Self) Self {
            const result = self.overflowing_add(rhs);
            return if (result.overflow) Self.MAX else result.result;
        }

        pub fn saturating_sub(self: Self, rhs: Self) Self {
            const result = self.overflowing_sub(rhs);
            return if (result.overflow) Self.ZERO else result.result;
        }
        ...
        pub fn reverse_bits(self: Self) Self {
            ...
            for (self.limbs, 0..) |limb, i| {
                result.limbs[limbs - 1 - i] = @bitReverse(limb);
            }
            return result.masked();
        }

        pub fn most_significant_bits(self: Self) struct { bits: u64, exp: usize } {
            const n_bits = self.bit_len();
            if (n_bits == 0) {
                return .{ .bits = 0, .exp = 0 };
            }

            // Determine the position of the MSB within the highest limb
            const limb_idx = (n_bits - 1) / 64;
            const bit_in_limb = (n_bits - 1) % 64;
            const high_limb = self.limbs[limb_idx];

            // Extract the most significant 64 bits
            var bits: u64 = undefined;
            if (bit_in_limb < 63) {
                // MSB is not at the top of the limb, need to combine with lower limb
                bits = high_limb << @intCast(63 - bit_in_limb);
                if (limb_idx > 0) {
                    bits |= self.limbs[limb_idx - 1] >> @intCast(bit_in_limb + 1);
                }
            } else {
                bits = high_limb;
            }

            // Normalize to have MSB at position 63
            const leading = @clz(bits);
            bits <<= @intCast(leading);

            const exp = n_bits - 64 + leading;
            return .{ .bits = bits, .exp = exp };
        }

        pub fn approx_pow2(exp: f64) ?Self {
            if (exp < 0 or std.math.isNan(exp) or std.math.isInf(exp)) {
                return null;
            }

            const floor_exp = @floor(exp);
            const fract_exp = exp - floor_exp;

            // Convert floor_exp to usize
            if (floor_exp > @as(f64, @floatFromInt(bits))) {
                return null;
            }
            const int_exp = @as(usize, @intFromFloat(floor_exp));

            // Start with 2^floor_exp
            if (int_exp >= bits) {
                return null;
            }
            var result = Self.ONE.shl(int_exp);

            // Approximate 2^fract_exp using Taylor series
            // 2^x ≈ 1 + x*ln(2) + (x*ln(2))^2/2! + ...
            if (fract_exp > 0) {
                const ln2 = 0.6931471805599453;
                const x = fract_exp * ln2;
                
                // Use a few terms of the Taylor series
                var multiplier: f64 = 1.0 + x + (x * x) / 2.0 + (x * x * x) / 6.0;
                
                // Scale the result
                var scaled = result;
                const shift_amount = 53; // Use 53 bits of precision
                if (int_exp + shift_amount < bits) {
                    scaled = scaled.shl(shift_amount);
                    const factor = @as(u64, @intFromFloat(multiplier * @as(f64, @floatFromInt(@as(u64, 1) << shift_amount))));
                    scaled = scaled.mul(Self.from_u64(factor));
                    scaled = scaled.shr(shift_amount);
                    result = scaled;
                }
            }
        ...
        pub fn wrapping_shl(self: Self, amount: u64) Self {
            const shift = amount % bits;
            return self.shl(shift);
        }

        pub fn wrapping_shr(self: Self, amount: u64) Self {
            const shift = amount % bits;
            return self.shr(shift);
        }

        pub fn overflowing_shl(self: Self, amount: u64) struct { result: Self, overflow: bool } {
            if (amount >= bits) {
                return .{ .result = Self.ZERO, .overflow = !self.is_zero() };
            }
            
            // Check if any bits will be shifted out
            const overflow = if (amount > 0) !self.shr(bits - amount).is_zero() else false;
            
            return .{ .result = self.shl(amount), .overflow = overflow };
        }

        pub fn overflowing_shr(self: Self, amount: u64) struct { result: Self, overflow: bool } {
            if (amount >= bits) {
                return .{ .result = Self.ZERO, .overflow = !self.is_zero() };
            }
            
            // Check if any bits will be shifted out
            const overflow = if (amount > 0) !self.shl(bits - amount).is_zero() else false;
            
            return .{ .result = self.shr(amount), .overflow = overflow };
        }

        pub fn from_limbs_slice(slice: []const u64) Self {
            const result = Self.overflowing_from_limbs_slice(slice);
            if (result.overflow) {
        ...
        pub fn checked_from_limbs_slice(slice: []const u64) ?Self {
            const result = Self.overflowing_from_limbs_slice(slice);
            return if (result.overflow) null else result.result;
        }

        pub fn wrapping_from_limbs_slice(slice: []const u64) Self {
            return Self.overflowing_from_limbs_slice(slice).result;
        }

        pub fn overflowing_from_limbs_slice(slice: []const u64) struct { result: Self, overflow: bool } {
            var result = Self.ZERO;
            const copy_len = @min(slice.len, limbs);
            
            // Copy available limbs
            for (slice[0..copy_len], 0..) |limb, i| {
                result.limbs[i] = limb;
            }
            ...
            // Check if there are more limbs than we can store
            if (slice.len > limbs) {
                for (slice[limbs..]) |limb| {
                    if (limb != 0) {
                        overflow = true;
                        break;
                    }
            ...
            // Check if the highest limb exceeds the mask
            if (copy_len > 0 and copy_len == limbs and result.limbs[limbs - 1] > MASK) {
                overflow = true;
                result.limbs[limbs - 1] &= MASK;
            }
        ...
        pub fn saturating_from_limbs_slice(slice: []const u64) Self {
            const result = Self.overflowing_from_limbs_slice(slice);
            return if (result.overflow) Self.MAX else result.result;
        }

        pub fn wrapping_neg(self: Self) Self {
            return Self.ZERO.wrapping_sub(self);
        }

        pub fn checked_neg(self: Self) ?Self {
            return if (self.is_zero()) Self.ZERO else null;
        }

        pub fn overflowing_neg(self: Self) struct { result: Self, overflow: bool } {
            const result = self.wrapping_neg();
            return .{ .result = result, .overflow = !self.is_zero() };
        }
    };
}
...
test "Uint from_u64" {
    const U64 = Uint(64, 1);
    const u1 = U64.from_u64(42);
    try testing.expectEqual(@as(u64, 42), u1.limbs[0]);

    const U128 = Uint(128, 2);
    const u2 = U128.from_u64(12345);
    try testing.expectEqual(@as(u64, 12345), u2.limbs[0]);
    try testing.expectEqual(@as(u64, 0), u2.limbs[1]);

    const U256 = Uint(256, 4);
    const u3 = U256.from_u64(std.math.maxInt(u64));
    try testing.expectEqual(@as(u64, std.math.maxInt(u64)), u3.limbs[0]);
    for (1..4) |i| {
        try testing.expectEqual(@as(u64, 0), u3.limbs[i]);
    }
}

test "Uint from_limbs" {
    const U128 = Uint(128, 2);
    const u1 = U128.from_limbs(.{ 0x1234567890ABCDEF, 0xFEDCBA0987654321 });
    try testing.expectEqual(@as(u64, 0x1234567890ABCDEF), u1.limbs[0]);
    try testing.expectEqual(@as(u64, 0xFEDCBA0987654321), u1.limbs[1]);

    const U256 = Uint(256, 4);
    const u2 = U256.from_limbs(.{ 1, 2, 3, 4 });
    try testing.expectEqual(@as(u64, 1), u2.limbs[0]);
    try testing.expectEqual(@as(u64, 2), u2.limbs[1]);
    try testing.expectEqual(@as(u64, 3), u2.limbs[2]);
    try testing.expectEqual(@as(u64, 4), u2.limbs[3]);
}
...
test "Uint masking for non-limb-aligned sizes" {
    ...
    const u1 = U65.from_limbs(.{ std.math.maxInt(u64), 0xFF });
    try testing.expectEqual(@as(u64, std.math.maxInt(u64)), u1.limbs[0]);
    try testing.expectEqual(@as(u64, 1), u1.limbs[1]);
...
test "div_ceil" {
    ...
    try testing.expectEqual(@as(u64, 4), a.div_ceil(b).to_u64().?);
    
    // Test division with remainder
    const c = U64.from_u64(21);
    const d = U64.from_u64(5);
    try testing.expectEqual(@as(u64, 5), c.div_ceil(d).to_u64().?); // ceil(21/5) = 5
    
    const e = U64.from_u64(19);
    try testing.expectEqual(@as(u64, 4), e.div_ceil(d).to_u64().?); // ceil(19/5) = 4
}
...
test "next_multiple_of" {
    ...
    // Test exact multiple
    try testing.expectEqual(@as(u64, 16), U64.from_u64(16).next_multiple_of(U64.from_u64(8)).to_u64().?);
    
    // Test rounding up
    try testing.expectEqual(@as(u64, 24), U64.from_u64(17).next_multiple_of(U64.from_u64(8)).to_u64().?);
    try testing.expectEqual(@as(u64, 24), U64.from_u64(23).next_multiple_of(U64.from_u64(8)).to_u64().?);
    
    // Test zero
    try testing.expectEqual(@as(u64, 0), U64.ZERO.next_multiple_of(U64.from_u64(5)).to_u64().?);
...
test "abs_diff" {
    ...
    const a = U64.from_u64(100);
    const b = U64.from_u64(30);
    
    try testing.expectEqual(@as(u64, 70), a.abs_diff(b).to_u64().?);
    try testing.expectEqual(@as(u64, 70), b.abs_diff(a).to_u64().?);
    
    // Same values
    try testing.expectEqual(U64.ZERO, a.abs_diff(a));
}
...
test "reverse_bits" {
    ...
    // Test single limb
    const a = U64.from_u64(0b1010101010101010000000000000000011111111111111110000000000000000);
    const reversed_a = a.reverse_bits();
    try testing.expectEqual(@as(u64, 0b0000000000000000111111111111111100000000000000001010101010101010), reversed_a.to_u64().?);
    
    // Test multiple limbs
    const b = U128.from_limbs(.{ 0xFFFF_FFFF_0000_0000, 0x0000_0000_FFFF_FFFF });
    const reversed_b = b.reverse_bits();
    try testing.expectEqual(@as(u64, 0xFFFF_FFFF_0000_0000), reversed_b.limbs[0]);
    try testing.expectEqual(@as(u64, 0x0000_0000_FFFF_FFFF), reversed_b.limbs[1]);
}

test "most_significant_bits" {
    const U256 = Uint(256, 4);
    
    // Test zero
    const zero = U256.ZERO;
    const msb_zero = zero.most_significant_bits();
    try testing.expectEqual(@as(u64, 0), msb_zero.bits);
    try testing.expectEqual(@as(usize, 0), msb_zero.exp);
    
    // Test power of 2
    const pow2 = U256.ONE.shl(100);
    const msb_pow2 = pow2.most_significant_bits();
    try testing.expectEqual(@as(u64, 1 << 63), msb_pow2.bits); // Normalized to MSB at position 63
    try testing.expectEqual(@as(usize, 37), msb_pow2.exp); // 101 - 64 = 37
    
    // Test value with multiple bits set
    const val = U256.from_int(0xFF00_0000_0000_0000);
    const msb_val = val.most_significant_bits();
    try testing.expect(msb_val.bits != 0);
}
...
test "approx_pow2" {
    ...
    try testing.expectEqual(U256.from_int(1), U256.approx_pow2(0).?);
    try testing.expectEqual(U256.from_int(2), U256.approx_pow2(1).?);
    try testing.expectEqual(U256.from_int(4), U256.approx_pow2(2).?);
    try testing.expectEqual(U256.from_int(1024), U256.approx_pow2(10).?);
    
    // Test fractional exponent
    const pow2_1_5 = U256.approx_pow2(1.5).?;
    // 2^1.5 ≈ 2.828, so result should be around 2 or 3
    try testing.expect(pow2_1_5.ge(U256.from_int(2)));
    try testing.expect(pow2_1_5.le(U256.from_int(3)));
    
    // Test invalid inputs
    try testing.expectEqual(@as(?U256, null), U256.approx_pow2(-1));
    try testing.expectEqual(@as(?U256, null), U256.approx_pow2(std.math.nan(f64)));
    try testing.expectEqual(@as(?U256, null), U256.approx_pow2(std.math.inf(f64)));
    try testing.expectEqual(@as(?U256, null), U256.approx_pow2(257)); // Too large for U256
}

test "wrapping shift operations" {
    const U64 = Uint(64, 1);
    
    const a = U64.from_u64(0xFF);
    
    // Test wrapping_shl
    try testing.expectEqual(U64.from_u64(0xFF00), a.wrapping_shl(8));
    try testing.expectEqual(U64.from_u64(0xFF00), a.wrapping_shl(64 + 8)); // Wraps around
    try testing.expectEqual(U64.from_u64(0xFF), a.wrapping_shl(64)); // Full rotation
    
    // Test wrapping_shr
    const b = U64.from_u64(0xFF00);
    try testing.expectEqual(U64.from_u64(0xFF), b.wrapping_shr(8));
    try testing.expectEqual(U64.from_u64(0xFF), b.wrapping_shr(64 + 8)); // Wraps around
    try testing.expectEqual(U64.from_u64(0xFF00), b.wrapping_shr(64)); // Full rotation
}

test "overflowing shift operations" {
    const U64 = Uint(64, 1);
    
    const a = U64.from_u64(0xFF);
    
    // Test overflowing_shl
    var result = a.overflowing_shl(8);
    try testing.expectEqual(U64.from_u64(0xFF00), result.result);
    try testing.expectEqual(false, result.overflow);
    
    result = a.overflowing_shl(57);
    try testing.expectEqual(U64.from_u64(0xFE00_0000_0000_0000), result.result);
    try testing.expectEqual(true, result.overflow); // Top bit shifted out
    
    result = a.overflowing_shl(64);
    try testing.expectEqual(U64.ZERO, result.result);
    try testing.expectEqual(true, result.overflow);
    
    // Test overflowing_shr
    const b = U64.from_u64(0xFF00_0000_0000_0000);
    result = b.overflowing_shr(8);
    try testing.expectEqual(U64.from_u64(0x00FF_0000_0000_0000), result.result);
    try testing.expectEqual(false, result.overflow);
    
    result = b.overflowing_shr(57);
    try testing.expectEqual(U64.from_u64(0x7F), result.result);
    try testing.expectEqual(true, result.overflow); // Bottom bit shifted out
    
    result = b.overflowing_shr(64);
    try testing.expectEqual(U64.ZERO, result.result);
    try testing.expectEqual(true, result.overflow);
}

test "from_limbs_slice operations" {
    const U128 = Uint(128, 2);
    
    // Test normal case
    const slice1 = [_]u64{ 0xFFFF_FFFF_FFFF_FFFF, 0x1234_5678_9ABC_DEF0 };
    const a = U128.from_limbs_slice(&slice1);
    try testing.expectEqual(@as(u64, 0xFFFF_FFFF_FFFF_FFFF), a.limbs[0]);
    try testing.expectEqual(@as(u64, 0x1234_5678_9ABC_DEF0), a.limbs[1]);
    
    // Test with smaller slice
    const slice2 = [_]u64{0xFFFF_FFFF_FFFF_FFFF};
    const b = U128.from_limbs_slice(&slice2);
    try testing.expectEqual(@as(u64, 0xFFFF_FFFF_FFFF_FFFF), b.limbs[0]);
    try testing.expectEqual(@as(u64, 0), b.limbs[1]);
    
    // Test checked_from_limbs_slice with overflow
    const slice3 = [_]u64{ 0xFFFF_FFFF_FFFF_FFFF, 0xFFFF_FFFF_FFFF_FFFF, 0x1 };
    try testing.expectEqual(@as(?U128, null), U128.checked_from_limbs_slice(&slice3));
    
    // Test wrapping_from_limbs_slice
    const c = U128.wrapping_from_limbs_slice(&slice3);
    try testing.expectEqual(@as(u64, 0xFFFF_FFFF_FFFF_FFFF), c.limbs[0]);
    try testing.expectEqual(@as(u64, 0xFFFF_FFFF_FFFF_FFFF), c.limbs[1]);
    
    // Test saturating_from_limbs_slice
    const d = U128.saturating_from_limbs_slice(&slice3);
    try testing.expectEqual(U128.MAX, d);
}
...
test "negation operations" {
    ...
    const a = U64.from_u64(100);
    const neg_a = a.wrapping_neg();
    try testing.expectEqual(U64.from_u64(0).sub(U64.from_u64(100)), neg_a);
    
    // Test checked_neg
    try testing.expectEqual(U64.ZERO, U64.ZERO.checked_neg().?);
    try testing.expectEqual(@as(?U64, null), U64.ONE.checked_neg());
    
    // Test overflowing_neg
    var result = U64.ZERO.overflowing_neg();
    try testing.expectEqual(U64.ZERO, result.result);
    try testing.expectEqual(false, result.overflow);
    
    result = U64.ONE.overflowing_neg();
    try testing.expectEqual(U64.MAX, result.result);
    try testing.expectEqual(true, result.overflow);
}