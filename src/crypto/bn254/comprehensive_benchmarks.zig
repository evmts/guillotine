...
fn calculateStats(allocator: std.mem.Allocator, name: []const u8, measurements: []u64) !BenchmarkStats {
    ...
    const iqr = q3 - q1;
    const outlier_threshold = q3 + 2.0 * iqr; // More lenient than standard 1.5*IQR
...
fn benchFp6Add() void {
    ...
    const i = nextInputIndex();
    const j = (i + 1) % 1000;
    const fp6_a = Fp6Mont.init_from_int(test_inputs.fp_a[i].value, test_inputs.fp_b[i].value, test_inputs.fp_a[j].value, test_inputs.fp_b[j].value, test_inputs.fp_a[(i + 2) % 1000].value, test_inputs.fp_b[(i + 2) % 1000].value);
    const fp6_b = Fp6Mont.init_from_int(test_inputs.fp_a[(i + 3) % 1000].value, test_inputs.fp_b[(i + 3) % 1000].value, test_inputs.fp_a[(i + 4) % 1000].value, test_inputs.fp_b[(i + 4) % 1000].value, test_inputs.fp_a[(i + 5) % 1000].value, test_inputs.fp_b[(i + 5) % 1000].value);
    const result = fp6_a.add(&fp6_b);
    std.mem.doNotOptimizeAway(result);
}
...
fn benchFp6Mul() void {
    ...
    const fp6_a = Fp6Mont.init_from_int(test_inputs.fp_a[i].value, test_inputs.fp_b[i].value, test_inputs.fp_a[j].value, test_inputs.fp_b[j].value, test_inputs.fp_a[(i + 2) % 1000].value, test_inputs.fp_b[(i + 2) % 1000].value);
    const fp6_b = Fp6Mont.init_from_int(test_inputs.fp_a[(i + 3) % 1000].value, test_inputs.fp_b[(i + 3) % 1000].value, test_inputs.fp_a[(i + 4) % 1000].value, test_inputs.fp_b[(i + 4) % 1000].value, test_inputs.fp_a[(i + 5) % 1000].value, test_inputs.fp_b[(i + 5) % 1000].value);
    const result = fp6_a.mul(&fp6_b);
    std.mem.doNotOptimizeAway(result);
}

fn benchFp6Square() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
    const j = (i + 1) % 1000;
    const fp6_a = Fp6Mont.init_from_int(test_inputs.fp_a[i].value, test_inputs.fp_b[i].value, test_inputs.fp_a[j].value, test_inputs.fp_b[j].value, test_inputs.fp_a[(i + 2) % 1000].value, test_inputs.fp_b[(i + 2) % 1000].value);
    const result = fp6_a.square();
    std.mem.doNotOptimizeAway(result);
}
...
fn benchFp6Inv() void {
    ...
    const i = nextInputIndex();
    const j = (i + 1) % 1000;
    const fp6_a = Fp6Mont.init_from_int(test_inputs.fp_nonzero[i].value, test_inputs.fp_nonzero[j].value, test_inputs.fp_nonzero[(i + 2) % 1000].value, test_inputs.fp_nonzero[(i + 3) % 1000].value, test_inputs.fp_nonzero[(i + 4) % 1000].value, test_inputs.fp_nonzero[(i + 5) % 1000].value);
    const result = fp6_a.inv() catch unreachable;
    std.mem.doNotOptimizeAway(result);
}

fn benchFp12Add() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
...
pub fn benchG1Add() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
    const j = (i + 1) % 1000;
    const result = test_inputs.g1_points[i].add(&test_inputs.g1_points[j]);
    std.mem.doNotOptimizeAway(result);
}

fn benchG1Double() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
...
fn benchG1ScalarMul() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
    const result = test_inputs.g1_points[i].mul(&test_inputs.fr_a[i]);
    std.mem.doNotOptimizeAway(result);
}
...
fn benchG2Add() void {
    ...
    const i = nextInputIndex();
    const j = (i + 1) % 1000;
    const result = test_inputs.g2_points[i].add(&test_inputs.g2_points[j]);
    std.mem.doNotOptimizeAway(result);
}

fn benchG2Double() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
...
fn benchG2ScalarMul() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
    const result = test_inputs.g2_points[i].mul(&test_inputs.fr_a[i]);
    std.mem.doNotOptimizeAway(result);
}

fn benchPairing() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
...
fn benchFinalExponentiation() void {
    const test_inputs = getInputs();
    const i = nextInputIndex();
    const result = pairing_mod.final_exponentiation(&test_inputs.fp12_a[i]);
    std.mem.doNotOptimizeAway(result);
}
...
pub fn runComprehensiveBenchmarks(allocator: std.mem.Allocator) !void {
    ...
    const benchmarks = [_]struct { name: []const u8, func: BenchmarkFunction, is_slow: bool }{
        // Field operations (fast)
        .{ .name = "Fp Addition", .func = benchFpAdd, .is_slow = false },
        .{ .name = "Fp Subtraction", .func = benchFpSub, .is_slow = false },
        .{ .name = "Fp Multiplication", .func = benchFpMul, .is_slow = false },
        .{ .name = "Fp Squaring", .func = benchFpSquare, .is_slow = false },
        .{ .name = "Fp Inversion", .func = benchFpInv, .is_slow = false },

        .{ .name = "Fp2 Addition", .func = benchFp2Add, .is_slow = false },
        .{ .name = "Fp2 Subtraction", .func = benchFp2Sub, .is_slow = false },
        .{ .name = "Fp2 Multiplication", .func = benchFp2Mul, .is_slow = false },
        .{ .name = "Fp2 Squaring", .func = benchFp2Square, .is_slow = false },
        .{ .name = "Fp2 Inversion", .func = benchFp2Inv, .is_slow = false },

        .{ .name = "Fp6 Addition", .func = benchFp6Add, .is_slow = false },
        .{ .name = "Fp6 Subtraction", .func = benchFp6Sub, .is_slow = false },
        .{ .name = "Fp6 Multiplication", .func = benchFp6Mul, .is_slow = false },
        .{ .name = "Fp6 Squaring", .func = benchFp6Square, .is_slow = false },
        .{ .name = "Fp6 Inversion", .func = benchFp6Inv, .is_slow = false },

        .{ .name = "Fp12 Addition", .func = benchFp12Add, .is_slow = false },
        .{ .name = "Fp12 Subtraction", .func = benchFp12Sub, .is_slow = false },
        .{ .name = "Fp12 Multiplication", .func = benchFp12Mul, .is_slow = false },
        .{ .name = "Fp12 Squaring", .func = benchFp12Square, .is_slow = false },
        .{ .name = "Fp12 Inversion", .func = benchFp12Inv, .is_slow = false },

        // Curve operations (some slow)
        .{ .name = "G1 Addition", .func = benchG1Add, .is_slow = false },
        .{ .name = "G1 Doubling", .func = benchG1Double, .is_slow = false },
        .{ .name = "G1 Scalar Multiplication", .func = benchG1ScalarMul, .is_slow = true },

        .{ .name = "G2 Addition", .func = benchG2Add, .is_slow = false },
        .{ .name = "G2 Doubling", .func = benchG2Double, .is_slow = false },
        .{ .name = "G2 Scalar Multiplication", .func = benchG2ScalarMul, .is_slow = true },

        // Pairing operations (slow)
        .{ .name = "Full Pairing", .func = benchPairing, .is_slow = true },
        .{ .name = "Miller Loop", .func = benchMillerLoop, .is_slow = true },
        .{ .name = "Final Exponentiation", .func = benchFinalExponentiation, .is_slow = true },
    };
    ...
    for (benchmarks) |bench_spec| {
        ...
        const stats = try benchmark(allocator, bench_spec.name, bench_spec.func, time_budget);
...
test "BN254 Quick Performance Check" {
    ...
    const quick_benchmarks = [_]struct { name: []const u8, func: BenchmarkFunction }{
        .{ .name = "Fp Multiplication", .func = benchFpMul },
        .{ .name = "G1 Addition", .func = benchG1Add },
        .{ .name = "Full Pairing", .func = benchPairing },
    };
    ...
    for (quick_benchmarks) |bench_spec| {
        const stats = try benchmark(std.testing.allocator, bench_spec.name, bench_spec.func, quick_time_budget);
...
test "BN254 Format Test" {
    // Initialize pre-generated inputs
    _ = getInputs();

    std.debug.print("\nBN254 Benchmark Formatting Test:\n", .{});
    std.debug.print("┌──────────────────────────────┬────────┬────────┬────────┬────────┬───────────┬────────┐\n", .{});
    std.debug.print("│ Operation                    │  Mean  │ StdDev │   P99  │   Max  │   Ops/s   │ Samples│\n", .{});
    std.debug.print("├──────────────────────────────┼────────┼────────┼────────┼────────┼───────────┼────────┤\n", .{});

    const format_benchmarks = [_]struct { name: []const u8, func: BenchmarkFunction }{
        .{ .name = "Fp Multiplication", .func = benchFpMul },
        .{ .name = "G1 Addition", .func = benchG1Add },
        .{ .name = "Full Pairing", .func = benchPairing },
    };

    const format_time_budget = 50000000; // 50ms for format test
    for (format_benchmarks) |bench_spec| {
        const stats = try benchmark(std.testing.allocator, bench_spec.name, bench_spec.func, format_time_budget);
        stats.format();
    }
