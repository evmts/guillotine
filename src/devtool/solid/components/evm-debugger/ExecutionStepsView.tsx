						...
						<TableRow>
							...
							<TableHead class="text-xs uppercase">
								<div class="flex items-center gap-1">
									...
									<InfoTooltip>
										Static per-block cost; excludes dynamic overhead like memory
										expansion, cold/warm storage, and per-word copy.
									</InfoTooltip>
								</div>
							</TableHead>
							<TableHead class="text-xs uppercase">
								<div class="flex items-center gap-1">
									...
									<InfoTooltip>
										Per-instruction runtime overhead. "-" until executed, then
										"+N" once known. "0" for ops with no dynamic overhead.
									</InfoTooltip>
								</div>
							</TableHead>
										...
										<div class="flex flex-col gap-2">
											<For each={blk.dynamicGas}>
												{(v, idx) => {
													...
													const isPast =
														insnIndex < props.currentInstructionIndex;
													const display =
														v > 0 ? `+${v}` : isPast ? "0" : canDyn ? "-" : "0";
													const muted = v === 0 && !isPast;
													return (
														<span
															class={cn(
																"flex items-center h-5.5",
									...
									<TableCell class="align-top">
										<div class="flex flex-col gap-2">
											<For each={blk.pcs}>
												{(pc) => {
													return (
														<Code class="inline-block w-fit text-xs">
															0x{pc.toString(16)}
														</Code>
													);
												}}
