...
export default function BytecodeBlocksMap(props: BytecodeBlocksMapProps) {
	...
	const hex = createMemo(() => (props.codeHex?.startsWith('0x') ? props.codeHex.slice(2) : props.codeHex || ''))
	...
	return (
		<div class="rounded-sm border border-border/40 bg-muted/30 p-3">
			<div class="mb-2 flex items-center justify-between">
				<div class="font-medium text-sm">Bytecode</div>
				<InfoTooltip>
					Each cell is one byte of the original bytecode. Cells are grouped by preanalized blocks. The current block
					being executed is highlighted.
				</InfoTooltip>
			</div>
			<div
				class="grid gap-0 overflow-hidden rounded-sm border border-border/30 font-mono text-xs tabular-nums"
