...
/// Threaded program containing function pointers for tailcall dispatch
pub const ThreadedProgram = struct {
    ops: []TailcallExecutionFunc,
    allocator: std.mem.Allocator,

    pub fn deinit(self: *ThreadedProgram) void {
        self.allocator.free(self.ops);
    }
};

/// Build a threaded program from analyzed bytecode
pub fn build(allocator: std.mem.Allocator, analysis: *const CodeAnalysis) !ThreadedProgram {
    // Allocate ops array matching instruction count
    const ops = try allocator.alloc(TailcallExecutionFunc, analysis.instructions.len);
...
fn mapTagToWrapper(tag: Tag) TailcallExecutionFunc {
    return switch (tag) {
