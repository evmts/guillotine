...
pub const PrestateTracer = struct {
    ...
    pub fn onStorageRead(self: *Self, address: Address, slot: u256, value: u256, is_warm: bool) void {
        ...
        self.state_changes.append(self.allocator, change) catch {};

        if (!self.disable_storage) {
            self.touchAccount(address);
    ...
    pub fn onCodeChange(self: *Self, address: Address, old_code: []const u8, new_code: []const u8) void {
        ...
        const change = StateChange{
            .step_number = self.total_instructions,
            .address = address,
            .change_type = .{ .code = .{ .old_code = old_code, .new_code = new_code, .old_code_hash = hashCode(old_code), .new_code_hash = hashCode(new_code) } },
        };
        self.state_changes.append(self.allocator, change) catch {};
        ...
        if (!self.disable_code) {
            ...
            if (!account.exists) {
                ...
                account.code_hash = hashCode(old_code);
                account.exists = true;
            }
        }
    }
    ...
    // ===== Helpers =====
    fn touchAccount(self: *Self, address: Address) void {
    ...
    fn buildPoststate(self: *Self) void {
            ...
            switch (change.change_type) {
                ...
                .account_created => |a| {
                    ...
                    if (!self.disable_code and a.code.len > 0) {
                        ...
                        got.value_ptr.code_hash = hashCode(a.code);
                    }
...
pub fn writePrestateJson(writer: anytype, tracer: *const PrestateTracer) !void {
    if (tracer.isDiffMode()) {
        ...
        try writeStateMapJson(writer, tracer.getPrestate(), tracer.isStorageDisabled(), tracer.isCodeDisabled());
        try writer.writeAll("},\"post\":{");
        try writeStateMapJson(writer, tracer.getPoststate(), tracer.isStorageDisabled(), tracer.isCodeDisabled());
        try writer.writeAll("}}");
    } else {
...
test "PrestateTracer memory management" {
    ...
    tracer.onCodeRead(addr, "test code");
    tracer.onTransactionEnd();
}
...
test "prestate tracer integrated with frame interpreter execution" {
    ...
    const bytecode = [_]u8{
        ...
        // 13. SELFDESTRUCT (to test onAccountDestroyed)
    ...
    const TestTracerWrapper = struct {
        ...
        pub fn onStorageRead(self: *@This(), address: Address, slot: u256, value: u256, is_warm: bool) void {
            self.prestate_tracer.onStorageRead(address, slot, value, is_warm);
        }

        pub fn onStorageWrite(self: *@This(), address: Address, slot: u256, old_value: u256, new_value: u256, is_warm: bool) void {
            self.prestate_tracer.onStorageWrite(address, slot, old_value, new_value, is_warm);
        }

        pub fn onBalanceRead(self: *@This(), address: Address, balance: u256) void {
            self.prestate_tracer.onBalanceRead(address, balance);
        }

        pub fn onCodeRead(self: *@This(), address: Address, code: []const u8) void {
            self.prestate_tracer.onCodeRead(address, code);
        }

        pub fn onAccountDestroyed(self: *@This(), address: Address, beneficiary: Address, balance_transferred: u256, had_code: bool, storage_cleared: bool) void {
            self.prestate_tracer.onAccountDestroyed(address, beneficiary, balance_transferred, had_code, storage_cleared);
        }
    };
    ...
    var interpreter = try FrameInterpreterType.init(allocator, &bytecode, 100000, // Same gas as working test
        db_interface, evm_mod.DefaultHost.init() // Use DefaultHost like the working test!
    );
