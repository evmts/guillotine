...
pub fn op_sload(context: *ExecutionContext) ExecutionError.Error!void {
    std.debug.assert(context.stack.size() >= 1);

    const slot = context.stack.peek_unsafe().*;

    if (context.chain_rules.is_berlin) {
        const is_cold = context.mark_storage_slot_warm(slot) catch {
    ...
    const value = context.get_storage(slot);

    context.stack.set_top_unsafe(value);
}

/// SSTORE opcode - Store value in persistent storage
pub fn op_sstore(context: *ExecutionContext) ExecutionError.Error!void {
    ...
    if (context.chain_rules.is_istanbul and context.gas_remaining <= GasConstants.SstoreSentryGas) {
        @branchHint(.unlikely);
    ...
    const current_value = context.get_storage(slot);
    ...
    const hardfork = context.chain_rules.getHardfork();
    const cost = storage_costs.calculateStorageCost(hardfork, current_value, value);
    total_gas += cost.gas;

    // Consume all gas at once
    try context.consume_gas(total_gas);

    try context.set_storage(slot, value);
    
    // Apply refund if any
    if (cost.refund > 0) {
        context.add_gas_refund(cost.refund);
    }
}
...
pub fn op_tload(context: *ExecutionContext) ExecutionError.Error!void {
    ...
    const value = context.get_transient_storage(slot);
...
pub fn op_tstore(context: *ExecutionContext) ExecutionError.Error!void {
    ...
    try context.set_transient_storage(slot, value);
}
