...
pub fn fuzz_bitwise_operations(allocator: std.mem.Allocator, operations: []const FuzzBitwiseOperation) !void {
    ...
    for (operations) |op| {
        ...
        if (op.op_type == BitwiseOpType.and_op) {
            result = try bitwise.op_and(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.or_op) {
            result = try bitwise.op_or(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.xor_op) {
            result = try bitwise.op_xor(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.not_op) {
            result = try bitwise.op_not(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.byte_op) {
            result = try bitwise.op_byte(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.shl_op) {
            result = try bitwise.op_shl(0, @ptrCast(&vm), @ptrCast(&frame));
        } else if (op.op_type == BitwiseOpType.shr_op) {
            result = try bitwise.op_shr(0, @ptrCast(&vm), @ptrCast(&frame));
