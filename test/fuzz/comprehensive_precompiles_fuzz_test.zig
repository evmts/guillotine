...
fn create_evm_context_for_precompiles(allocator: std.mem.Allocator) !struct {
    ...
    var contract = evm.Contract.init(primitives.Address.ZERO, primitives.Address.ZERO, 0, 10000000, // High gas limit for precompile calls
        &call_precompile_code, [_]u8{0} ** 32, &.{}, false);
...
test "fuzz_ecrecover_precompile_edge_cases" {
    ...
    for (ecrecover_tests) |test_case| {
        ...
        const result = evm.precompiles.ecrecover.execute(test_case.input_data, &output_buffer, test_case.expected_gas + 1000 // Provide enough gas
        );
...
test "fuzz_identity_precompile_edge_cases" {
    ...
    for (identity_tests) |test_case| {
        ...
        const result = evm.precompiles.identity.execute(input_data, output_buffer, test_case.expected_gas + 100 // Provide enough gas
        );
...
test "fuzz_sha256_precompile_edge_cases" {
    ...
    for (sha256_tests) |test_case| {
        ...
        for (patterns) |pattern| {
            ...
            const result = evm.precompiles.sha256.execute(input_data, &output_buffer, test_case.expected_gas + 100 // Provide enough gas
            );
            ...
            if (result == .success) {
                ...
                const second_result = evm.precompiles.sha256.execute(input_data, &second_output, test_case.expected_gas + 100);
...
test "fuzz_ripemd160_precompile_edge_cases" {
    ...
    for (ripemd160_tests) |test_case| {
        ...
        const result = evm.precompiles.ripemd160.execute(input_data, &output_buffer, test_case.expected_gas + 100 // Provide enough gas
        );
        ...
        if (result == .success) {
            ...
            var second_output: [32]u8 = undefined;
            const second_result = evm.precompiles.ripemd160.execute(input_data, &second_output, test_case.expected_gas + 100);
            try testing.expect(second_result.is_success());
            try testing.expectEqualSlices(u8, &output_buffer, &second_output);
        }
    }
    ...
    for (deterministic_tests) |test_case| {
        ...
        if (std.mem.eql(u8, test_case.precompile_name, "IDENTITY")) {
            ...
            if (result1 == .success and result2 == .success) {
                ...
                try testing.expectEqualSlices(u8, output1[0..result1.success.output_size], output2[0..result2.success.output_size]);
            }
        } else if (std.mem.eql(u8, test_case.precompile_name, "SHA256")) {
