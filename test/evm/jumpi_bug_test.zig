...
test "WORKING dynamic JUMPI to valid JUMPDEST returns 0x01" {
    ...
    var vm = try Evm.init(allocator, db_interface, null, null, null, null, null);
    defer vm.deinit();
...
test "JUMPI should take jump when condition is non-zero" {
    ...
    const db_interface = memory_db.to_database_interface();
    var vm2 = try Evm.init(allocator, db_interface, null, null, null, null, null);
    defer vm2.deinit();
...
test "JUMPI should NOT jump when condition is zero" {
    ...
    const db_interface = memory_db.to_database_interface();
    var vm3 = try Evm.init(allocator, db_interface, null, null, null, null, null);
    defer vm3.deinit();
