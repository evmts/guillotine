...
test "Integration: memory expansion limits" {
    ...
    try vm.table.execute(&vm, &frame, 0x52);
    ...
    const result = vm.table.execute(&vm, &frame, 0x52);
...
test "Integration: signed arithmetic boundaries" {
    ...
    try vm.table.execute(&vm, &frame, 0x05);
...
test "Integration: bitwise operation boundaries" {
    ...
    try vm.table.execute(&vm, &frame, 0x1B);
    ...
    try vm.table.execute(&vm, &frame, 0x1C);
    try testing.expectEqual(@as(u256, 0), try frame.stack.pop()); // Shifts out completely
    ...
    try vm.table.execute(&vm, &frame, 0x1D);
    ...
    try vm.table.execute(&vm, &frame, 0x1A);
...
test "Integration: call gas calculation edge cases" {
    ...
    try vm.table.execute(&vm, &frame, 0xF1);
...
test "Integration: return data boundary conditions" {
    ...
    try vm.table.execute(&vm, &frame, 0x3E);
    ...
    const result = vm.table.execute(&vm, &frame, 0x3E);
    ...
    const result2 = vm.table.execute(&vm, &frame, 0x3E);
...
test "Integration: exponentiation edge cases" {
    ...
    try vm.table.execute(&vm, &frame, 0x0A);
...
test "Integration: jump destination validation" {
    ...
    try frame.stack.push(1);
    const result1 = vm.table.execute(&vm, &frame, 0x56);
    ...
    const result2 = try vm.table.execute(&vm, &frame, 0x56);
    _ = result2;
...
test "Integration: storage slot temperature transitions" {
    ...
    try vm.table.execute(&vm, &frame, 0x54);
...
test "Integration: MCOPY overlap handling" {
    ...
    try vm.table.execute(&vm, &frame, 0x5E);
