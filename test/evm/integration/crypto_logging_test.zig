...
test "Integration: SHA3 with dynamic data" {
    ...
    try frame.stack_push(data1); // value
    ...
    try frame.stack_push(data2); // value
    ...
    const hash = try frame.stack_pop();
    ...
    const empty_hash = try frame.stack_pop();
...
test "Integration: Logging with topics and data" {
    ...
    try frame.memory_set_data(0, &event_data);
    ...
    try frame.stack_push(0); // offset
    try frame.stack_push(32); // size
    try frame.stack_push(transfer_sig); // topic1: Transfer signature

    // Execute LOG1
    _ = try vm.table.execute(&vm, &frame, 0xA1);
...
test "Integration: LOG operations with multiple topics" {
    ...
    try frame.stack_push(topic1); // topic1
    try frame.stack_push(topic2); // topic2
    try frame.stack_push(topic3); // topic3
    ...
    // Execute LOG0
    _ = try vm.table.execute(&vm, &frame, 0xA0);
...
test "Integration: Hash-based address calculation" {
    ...
    // 0xff prefix
    try frame.memory_set_data(offset, &[_]u8{0xff});
    offset += 1;

    // Deployer address (20 bytes)
    const deployer_bytes = contract_address.bytes;
    try frame.memory_set_data(offset, &deployer_bytes);
    offset += 20;
    ...
    try frame.memory_set_data(offset, &salt_bytes);
    offset += 32;
    ...
    try frame.memory_set_data(offset, &hash_bytes);

    // Hash all components (1 + 20 + 32 + 32 = 85 bytes)
    try frame.stack_push(0); // offset
    try frame.stack_push(85); // size

    // Execute SHA3
    _ = try vm.table.execute(&vm, &frame, 0x20);

    // Extract address from hash (last 20 bytes)
    const full_hash = try frame.stack_pop();
...
test "Integration: Event emission patterns" {
    ...
    try frame.memory_set_data(0, &value_bytes);

    // Emit Transfer event with LOG3
    const transfer_sig: u256 = 0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef;

    // Push stack values in reverse order
    try frame.stack_push(0); // offset in memory
    try frame.stack_push(32); // size of data (value)
    try frame.stack_push(transfer_sig); // topic1: event signature
    try frame.stack_push(from_addr); // topic2: indexed 'from'
    try frame.stack_push(to_addr); // topic3: indexed 'to'
    ...
    try frame.memory_set_data(32, &allowance_bytes);

    const approval_sig: u256 = 0x8c5be1e5ebec7d5bd14f71427d1e84f3dd0314c0f7b2291e5b200ac8c7c3b925;

    // Push stack values in reverse order
    try frame.stack_push(32); // offset in memory
    try frame.stack_push(32); // size of data (allowance)
    try frame.stack_push(approval_sig); // topic1: event signature
    try frame.stack_push(owner_addr); // topic2: indexed 'owner'
    try frame.stack_push(spender_addr); // topic3: indexed 'spender'
...
test "Integration: Dynamic log data with memory expansion" {
    ...
    try frame.memory_set_data(high_offset, message);

    // Check memory size before
    _ = try vm.table.execute(&vm, &frame, 0x59);
    const size_before = try frame.stack_pop();

    // Log with data from high offset
    try frame.stack_push(high_offset); // offset
    try frame.stack_push(message.len); // size
    try frame.stack_push(0x1234567890ABCDEF); // topic1
    ...
    _ = try vm.table.execute(&vm, &frame, 0x59);
    const size_after = try frame.stack_pop();
...
test "Integration: SHA3 for signature verification" {
    ...
    try frame.memory_set_data(0, function_sig);

    // Hash the function signature
    try frame.stack_push(0); // offset
    try frame.stack_push(function_sig.len); // size
...
test "Integration: Log in static context fails" {
    ...
    const result = opcodes.log.op_log0(&vm, &frame);
    ...
    try frame.stack_push(0); // offset
    try frame.stack_push(0); // size
    try frame.stack_push(0x1111); // topic

    const result1 = opcodes.log.op_log1(&vm, &frame);
