...
test "complex: fibonacci calculation" {
    ...
    // fibonacci(10) = 55
    const expected_bytes = u256_to_bytes32(55);
...
test "complex: call depth limit enforcement" {
    ...
    // evm.last_frame is no longer available, so we can't directly set depth.
    ...
    // try testing.expectEqual(@as(u256, 1), evm.last_stack_value.?);
    //
    ...
    // try testing.expectEqual(@as(u256, 0), evm.last_stack_value.?);
}
