...
test "MSIZE (0x59): Get current memory size" {
    ...
    _ = try evm.table.execute(interpreter, state, 0x53); // MSTORE8
...
test "GAS (0x5A): Get remaining gas" {
    ...
    for (test_cases) |initial_gas| {
        ...
        // Execute GAS opcode
        _ = try evm.table.execute(interpreter, state, 0x5A);
        ...
        _ = try evm.table.execute(interpreter, state, 0x01); // ADD (costs 3)
        _ = try frame.stack.pop();
...
test "MSIZE: Memory expansion scenarios" {
    ...
    _ = try evm.table.execute(interpreter, state, 0x37); // CALLDATACOPY
...
test "GAS: Low gas scenarios" {
    ...
    const result = evm.table.execute(interpreter, state, 0x5A);
...
test "JUMPDEST: Code analysis integration" {
    ...
    _ = try evm.table.execute(interpreter, state, 0x56); // JUMP
